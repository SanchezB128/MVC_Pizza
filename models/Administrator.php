...
class Administrator extends db{
	...
	private function register_users($data){
		try {
			$SQL = 'INSERT INTO tbl_ingredientes (nombre,descripcion,precio,unidad_mdd,stock,fecha_cad,cnt_ing) VALUES (?,?,?,?,?,?,?)';
			$result = $this->connect()->prepare($SQL);
	...
	private function update_user($data){
		try {
			$SQL = 'UPDATE tbl_ingredientes SET nombre = ?, descripcion = ?, precio = ?, unidad_mdd = ?, stock = ?, fecha_cad = ?, cnt_ing = ? WHERE id = ?';
			$result = $this->connect()->prepare($SQL);
			$result->execute(array(
								$data['nombre'],
								$data['descripcion'],
	...
	private function delete_user($id){
		try {
			$SQL = 'DELETE FROM tbl_ingredientes WHERE id = ?';
			$result = $this->connect()->prepare($SQL);
